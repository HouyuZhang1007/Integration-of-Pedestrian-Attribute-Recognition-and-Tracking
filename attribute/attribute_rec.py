...
class AttributeRec:
	...
	def __init__(self, gpu_id, model = 'VESPA_PA100K_26'):
		...
		if model == 'VESPA_PA100K_26':
			self.net = caffe.Net('./attribute/models/VesPA_PA100K_26/deploy_pa100k.prototxt', './attribute/models/VesPA_PA100K_26/vespa-pa100k_iter_35000.caffemodel', caffe.TEST)
			self.data = sio.loadmat('./attribute/PA100K_annotation.mat')
			self.name = self.data['attributes']
			self.attr_group = [range(1, 4), range(4, 7), range(13, 15), range(15, 18)]
			self.threshold = np.ones(26) * 0.5
			self.mean = self.load_mean('attribute/models/VesPA_RAP_51/rap_mean.binaryproto')[0,:]
			self.mean = self.mean.transpose((1,2,0)) #c, h, w change to h, w, c

		elif model == 'VESPA_RAP_51':
			self.net = caffe.Net('./attribute/models/VesPA_RAP_51/deploy_rap.prototxt', './attribute/models/VesPA_RAP_51/vespa-rap_iter_16000.caffemodel', caffe.TEST)
			self.data = sio.loadmat('./attribute/RAP_annotation.mat')['RAP_annotation']
			...
			self.net = caffe.Net('./attribute/models/VesPA_RAP_18/deploy_rap.prototxt', './attribute/models/VesPA_RAP_18/vespa-rap18_iter_60000.caffemodel', caffe.TEST)
			self.data = sio.loadmat('./attribute/RAP_annotation.mat')['RAP_annotation']
			...
			self.net = caffe.Net('./attribute/models/WPAL_GOOGLENET_PA100K_26/test_net.prototxt', './attribute/models/WPAL_GOOGLENET_PA100K_26/googlenet_spp_PA-100K_iter_170000.caffemodel', caffe.TEST)
			self.data = sio.loadmat('./attribute/PA100K_annotation.mat')
			self.name = self.data['attributes']
			self.attr_group = [range(1, 4), range(4, 7), range(13, 15), range(15, 18)]
			self.threshold = np.ones(26) * 0.5
...
if __name__ == '__main__':
	...
	imgname.append("./image/0090_a.png")
	device_mode = -1 #-1 for cpu or n for gpu device
	a = AttributeRec(device_mode)
	attr, orig_pred = a.attr_rec(imgname)
